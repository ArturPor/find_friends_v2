...
# generowanie clastrowania
from openai import OpenAI

# Retrieve OpenAI API key from Streamlit secrets
if not st.secrets.get("OPENAI_API_KEY"):
    st.error("Brakuje klucza openai_api_key w pliku .streamlit/secrets.toml")
    st.stop()

# Assign to a descriptively named variable
# Używamy nazwy openai_api_key, by od razu było jasne, czego dotyczy
openai_api_key = st.secrets["OPENAI_API_KEY"]

# Cache the OpenAI client as a resource
@st.cache_resource
def get_openai_client():
    # Bezpośrednio przekazujemy nazwany argument api_key
...
with st.sidebar:
    st.header("Przedstaw się")
    st.markdown("Znajdźmy osoby o podobnych zainteresowaniach")
...
st.header(f"Twoja grupa to: {predicted_cluster_data['name']}")
st.markdown(predicted_cluster_data['description'])
same_cluster_df = all_df[all_df["Cluster"] == predicted_cluster_id]
st.metric("Liczba osób o podobnych zainteresowaniach", len(same_cluster_df))
